...
export default function HomepageExtras() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      // Smooth fade-up for all major sections
  ...
  return (
    <div ref={containerRef} className="w-full bg-white text-zinc-900">
      ...
      <section className="reveal-section py-32 px-8 md:px-24 bg-zinc-50 border-t border-rosegold/20">
        <h2 className="text-xs uppercase tracking-[0.3em] text-boxrose mb-16 text-center">Client Trust</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 max-w-6xl mx-auto">
          {testimonials.map((t, i) => (
            <article key={i} className="flex flex-col justify-between p-8 rounded-[2.5rem] bg-white/40 backdrop-blur-xl shadow-xl shadow-rosegold/5 transition-transform duration-500 hover:-translate-y-2" style={{ border: "1px solid transparent", background: "linear-gradient(white, white) padding-box, linear-gradient(to right, #b76e79, #a47774) border-box", borderRadius: "2.5rem" }}>
              <p className="text-lg font-light leading-relaxed text-zinc-700 mb-8">&ldquo;{t.quote}&rdquo;</p>
              <div>
                <p className="font-medium text-sm">{t.author}</p>
                <p className="text-xs text-zinc-500 mt-1">{t.title}</p>
              </div>
            </article>
          ))}
        </div>
      ...
      <section className="reveal-section py-32 px-8 md:px-24 max-w-4xl mx-auto">
        ...
        <div className="border-t border-rosegold/20">
          {faqs.map((faq, i) => (
            <div key={i} className="border-b border-rosegold/20 overflow-hidden">
              ...
              >
                <span className={`text-lg transition-colors duration-300 ${openFaq === i ? "text-[#b76e79]" : "text-zinc-600"}`}>
                  {faq.question}
                </span>
                <svg
              ...
              <div
                className={`transition-all duration-500 ease-in-out ${openFaq === i ? "max-h-40 opacity-100 pb-6" : "max-h-0 opacity-0"}`}
              >
                <p className="text-zinc-500 leading-relaxed pr-12">{faq.answer}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="reveal-section py-32 px-8 md:px-24 bg-zinc-50 border-t border-[#b76e79]/15 text-zinc-900 text-center">
        <div className="max-w-3xl mx-auto">
          <h2 className="text-4xl md:text-5xl font-light tracking-tight mb-6">Ready to initiate your next project?</h2>
          <p className="text-zinc-500 mb-10 text-lg">Connect with our architectural advisory board to discuss your portfolio requirements.</p>
