...
const GLSLHills = ({ width = '100vw', height = '100vh', cameraZ = 125, planeSize = 256, speed = 0.5 }) => {
  ...
  useEffect(() => {
      ...
      createMesh() {
        return new THREE.Mesh(
          ...
          new THREE.RawShaderMaterial({
            ...
            // (Vertex Shader remains mathematically identical to the original template)
            vertexShader: `
              #define GLSLIFY 1
              ...
              void main(void) {
                ...
                float noise1 = cnoise(noisePosition * 0.08); float noise2 = cnoise(noisePosition * 0.06); float noise3 = cnoise(noisePosition * 0.4);
                vec3 lastPosition = updatePosition + vec3(0.0, noise1 * sin1 * 8.0 + noise2 * sin1 * 8.0 + noise3 * (abs(sin1) * 2.0 + 0.5) + pow(sin1, 2.0) * 40.0, 0.0);
                vPosition = lastPosition; gl_Position = projectionMatrix * modelViewMatrix * vec4(lastPosition, 1.0);
              }
            `,
            // CHANGED: Fragment Shader injected with Rose Gold RGB values (0.717, 0.431, 0.474)
            fragmentShader: `
              ...
              void main(void) {
                ...
                // ROSE GOLD SHADER COLOR INJECTION
                vec3 color = vec3(0.717, 0.431, 0.474);
    ...
    const init = () => {
      renderer.setSize(window.innerWidth, window.innerHeight);
      renderer.setClearColor(0xffffff, 0); // Transparent over white background
